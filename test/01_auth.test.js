...
beforeAll(async () => {
  ...
  test("A user sign up, update his password with a token, and then login with the new password", async () => {
    ...
    const response3 = await request(app).post("/api/auth/login").send({ // se logea con la nuev contraseña
      email: "testuser@gmail.com",
      password: "newPassword",
    })
  ...
  test("User sign up and then delete his account succesfull", async () => {
    ...
    const response2 = await request(app).post("/api/auth/login").send({ // se logea para obtener token
      email: "usertodelete@admin.com",
      password: "adminuser",
    });
    expect(response2.statusCode).toEqual(200); 

    const response3 = await request(app).delete("/api/auth/users/").set( // borra el usuario
      "Authorization", "Bearer " + response2._body.token);
    expect(response3.statusCode).toEqual(200); 
